...
function SubCommentLikeList({onClose,subCommentId}) 
{
    const {data,error,postData} = useApiPost();
    const handleGetSubCommentLikeList = () => {
        try{
            const response = postData("/reel_subcomment_like_list",{reel_sub_comment_id:subCommentId})
        } catch(error){}
    }

    useEffect(() => {
        handleGetSubCommentLikeList()
    },[subCommentId])

    const SubCommentLikeList = data?.reel_subcomment_like_list 
    console.log("Reel Subcomment List !!",SubCommentLikeList)
...
export default SubCommentLikeList;
