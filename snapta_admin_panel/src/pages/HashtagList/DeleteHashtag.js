...
function DeleteHashtag({ open, handleClose, handleDelete, hashtagId }) {

    const {data,error,postData} = useApiPost()
    const handleDeleteHashtag = () => {
        try{
            const response = postData("/delete_hashtag",{hashtag_id:hashtagId})
            toast.success("Hahstag Word Deleted Successfully!")
            handleClose()
        } catch(error) {
    ...
    return (
        <Dialog open={open} onClose={handleClose}  fullWidth className="custom-dialog">
            <DialogContent className="flex flex-col items-center text-center">
                ...
                <p className="text-[#606060] font-poppins text-base mt-2 w-[310px]">
                    This hashtag word will be deleted permanently. You cannot undo this action.
                </p>
            </DialogContent>
            ...
            <DialogContent className="flex justify-center gap-4 pb-4">
                <div className="flex justify-center gap-3">
                    ...
                    <button onClick={handleDeleteHashtag} className="px-20 py-2 font-medium text-white rounded-lg bg-button-gradient" >
                        Delete
                    </button>
                </div>
            </DialogContent>
