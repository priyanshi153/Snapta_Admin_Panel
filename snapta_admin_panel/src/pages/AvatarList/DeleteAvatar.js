...
function DeleteAvatar({ open, handleClose, handleDelete, avtarId }) {

    const {data,error,postData} = useApiPost()
    const handleDeleteAvatar = () => {
        try{
            const response = postData("/delete_avtar",{avtar_id:avtarId})
        ...
        <Dialog open={open} onClose={handleClose}  fullWidth className="custom-dialog">
            ...
            <DialogContent className="flex justify-center gap-4 pb-4">
                <div className="flex justify-center gap-3">
                    ...
                    <button onClick={handleDeleteAvatar} className="px-20 py-2 font-medium text-white rounded-lg bg-button-gradient" >
                        Delete
                    </button>
                </div>
            </DialogContent>
