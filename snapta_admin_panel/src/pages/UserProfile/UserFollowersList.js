...
function Followers({open,handleClose}) 
{
    const {data,loading,error,refetch,postData} = useApiPost();
    const userId = Cookies.get("userId")
    const handleGetFollowers = async() => {
        try{
            const response = await postData("/second_user_followers",{to_user_id:userId})
        } catch(error){
    ...
    const FollowersList = data?.follower
  ...
  <h2 className="text-[#000000] font-semibold text-xl font-poppins dark:text-darkText">Followers</h2>
  <img
    src={Cross}
    alt="Close"
  ...
  />
        ...
        {FollowersList?.map((follower) => (
            <>
                    ...
                    <div className='flex'>
                        <img src={follower.profile_pic || "/assets/default_user.png"} alt={follower.username}  className="w-12 h-12 mr-3 rounded-full" />
                            <div className='flex flex-col py-2 '>
                                <p className='font-poppins text-[#000000] text-sm text-left font-semibold dark:text-darkText'> {follower.first_name} {follower.last_name}</p>
               ...
               <div className='flex flex-col justify-center place-items-center min-h-[400px] dark:bg-primary'>
                    <img src={Empty} className='w-16 h-16 '/>
                    <p className='text-[#000000] text-opacity-[59%] font-poppins flex justify-center dark:text-darkText'>No Followers found</p>
                </div>
            </>) }
