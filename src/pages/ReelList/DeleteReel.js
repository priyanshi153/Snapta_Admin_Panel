...
function DeleteReel({ open, handleClose, handleDelete, ReelId }) {
    const {data,error,postData} = useApiPost();

    const handleReelDelete = () => {
        try{
            const response = postData("/delete_reel",{reel_id:ReelId})
        ...
        <Dialog open={open} onClose={handleClose}  fullWidth className="custom-dialog">
            ...
            <DialogContent className="flex justify-center gap-4 pb-4">
                <div className="flex justify-center gap-3">
                    ...
                    <button onClick={handleReelDelete} className="px-20 py-2 font-medium text-white rounded-lg bg-button-gradient" >
                        Delete
                    </button>
                </div>
            </DialogContent>
