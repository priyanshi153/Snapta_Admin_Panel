...
function TotalPosts() 
{
    ...
    const {data:PostData,isLoading} = useGetAllTotalQuery({token:token})
    console.log("Story @@@",PostData)

    const PostRatio = PostData?.post_last_month

    return(
        <>
         <div className="border border-[#D1D5DB] dark:border-[#1F1F1F] rounded-lg px-4 py-5">
            <div className="flex justify-between py-1">
                <h2 className="font-poppins text-[#000000] font-semibold text-base dark:text-gray-500">Total Posts</h2>
                <h2 className={` ${PostRatio < 0 ? "text-[#EE6D3D] bg-[#FCECD6]" : "text-[#22973F] bg-[#D1F9DB]"} px-5 py-0.5 rounded-2xl font-medium flex gap-1 place-items-center`}>
                    {PostData?.post_last_month}
            ...
            <h1 className="text-[#000000] font-poppins text-2xl font-semibold dark:text-darkText">{PostData?.total_post}</h1>
            <p className="text-[#9B9B9B] font-poppins text-sm py-1">vs last month: <span className="text-[#000000] font-semibold text-sm dark:text-gray-500">{PostData?.post_permonth}</span></p>
         </div>
        </>
    )
}
