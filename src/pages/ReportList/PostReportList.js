...
const handleGetReportList = async() => {
  setIsLoading(true)
  try{
    const response = await postData("/get_all_post_reportlist_pagination",{page_no:activePage,per_page:selectedValue,order:order,category:category,search:search})
  } catch(error){}
  finally{
    setIsLoading(false)
  }
...
const AllPostReportList = data?.report_post || [];
  
const [isDropdownOpen, setIsDropdownOpen] = useState(false);
const dropdownRef = useRef(null);

console.log("Report @@@",AllPostReportList)
...
const url = AllPostReportList.map((report) => (report.post_images[0]))
console.log("Post Images !!!!",url)

const {data:PostReport,postData:postReportData} = useApiPost();



console.log("Post Status !!!",PostReport)

const [toggleStates, setToggleStates] = useState(() => {
  return AllPostReportList.reduce((acc, post) => {
...
useEffect(() => {
  if (AllPostReportList.length > 0) {
    const initialStates = AllPostReportList.reduce((acc, post) => {
...
const handlePostStatus = (postId) => {
  try{
    const response = postReportData("/post_edit_status",{post_id:postId})
...
const handleOpenUserProfile = (userId) => {
  ...
  return (
    ...
    <div className="light:mb-10 xl:pl-72 dark:bg-primary bg-secondary">
      ...
      <div className="flex justify-between border-t-[#F2F2F2] py-3 px-6">
        <h2 className="text-[#000000] dark:text-darkText font-poppins text-xl font-semibold pt-3">Post Report List</h2>
      ...
      <div className="flex items-center justify-between px-6 mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[#858585] font-poppins text-base">Post Report List</h3>
        </div>
      </div>
  ...
  {/* Table Header */}
  <div className="flex px-4 py-4 pl-4 text-left dark:border-[#1F1F1F] border-b bg-header sm:pl-16">
    ...
    {/* Post Image */}
    <div className='w-[15%] text-[#FFFFFF] font-poppins text-sm flex place-items-center gap-2'> POST IMAGE </div>
  ...
  {/* Data Rows */}
  {AllPostReportList.map((report, index) => (
    <div key={report.id} className={`${index % 2 === 0 ? 'bg-white dark:bg-primary' : 'bg-[#00162e0a] dark:bg-primary'} flex items-center px-4 py-3 sm:pl-16 pl-4 dark:border-b-[#1F1F1F] border-b last:border-0`}>
      ...
      <div className="w-[20%]">
        ...
        <h2 className="text-[#7A7A7A] font-poppins text-xs">{formatTime(report.created_at)}</h2> 
      </div>

      {/* Post Image */}
      <div className='w-[15%] cursor-pointer' onClick={() => handlePostDetailOpen(report.post_id)}>
        <img src={report.post_images[0]?.url} className="object-cover rounded-lg w-14 h-14" />
      </div>
      ...
      <div className="w-[25%] flex gap-2 items-center">
        ...
        <div>
          <h2 className="text-[#000000] dark:text-darkText font-poppins text-sm font-semibold cursor-pointer" onClick={() => handleOpenUserProfile(report.post_userid)}>{report.post_username}</h2>
          <p className="text-xs text-[#939393] font-poppins">{report.post_email}</p>
        </div>
      </div>

      {/* Reason */}
      <div className="w-[25%] text-[#939393] dark:text-darkText font-poppins text-sm">{report.report_text}</div>

      {/* Reported By */}
      <div className="w-[25%] flex gap-2 items-center">
        <img src={report.report_profile_pic} className="w-12 h-12 rounded-full"/>
        <div>
          <h2 className="text-[#000000] font-poppins text-sm dark:text-darkText font-semibold cursor-pointer" onClick={() => handleOpenUserProfile(report.user_id)}>{report.report_username}</h2>
          <p className="text-xs text-[#939393] dark:text-tableDarkLarge font-poppins">{report.report_email}</p>
        </div>
      </div>

      {/* status */}
      <div key={report.post_id} className="w-[10%]">
  <label className="flex items-center cursor-pointer select-none">
    <div className="relative">
      <input
        type="checkbox"
        checked={toggleStates[report.post_id]}
        onChange={() => {handlePostStatus(report.post_id)}}
        className="sr-only"
      />
      {/* Toggle Background */}
      <div
        className={`block h-6 w-10 rounded-full border transition duration-300 ${
          toggleStates[report.post_id]
